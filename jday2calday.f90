!***********************************************************************************************************************************
!
!  Program modified by Robert Green from   DOY2GREG  -  David G. Simpson   -   NASA Goddard Space Flight Center
!
...
! Read in the calendar date from standard input
      ...
      call get_command_argument(2,DOYarg)
      read(Yarg,'(I10)') Y
      read(DOYarg,'(I10)') DOY
!      write(*,*) Y,DOY
...
! Checking if there are too many days in the year
      ...
      IF (Y .GE. 1) THEN                                                            ! print results (AD)
         WRITE (UNIT=*, FMT='(I4,A,I2.2,A,I2.2)') Y,'-',M,'-',D 
!         WRITE (UNIT=*, FMT='(/,1X,A,1X,I2,", ",I7, " AD")')   &
!                TRIM(MONTH_NAME(M)), D, Y
      ELSE                                                                          ! print results (BC)
         WRITE (UNIT=*, FMT='(I4,A,I2.2,A,I2.2)') Y,'-',M,'-',D                        ! print result
